function Sim = BL_Analysis(Sim)
%%  Parameters
rho_fuel = 835;         %   kg/L - Fuel density
rho_air = 1.2;          %   kg/m^3 - Air density
r_wh = 0.5003;          %   m Wheel radius 
A_f = 10.67;            %   m^2 - Frontal area 
Cd = 0.49;              %   Drag coefficient 
%%  Fuel Calculations
Sim.t_tot = Sim.time(end)-Sim.time(1);                            %   s - Total time of drive 
Sim.P_eng = Sim.T_eng.*Sim.N_eng*pi/30;                     %   W - Engine power
...
Sim.fuel_tot = Sim.m_fuel_TOT(end)-Sim.m_fuel_TOT(1);                   %   g - Total fuel consumed
Sim.V_fuel = Sim.fuel_tot/rho_fuel*.264;                %   gal - Volume of fuel consumed
Sim.mpg = (Sim.X_veh(end)-Sim.X_veh(1))*6.21e-4/Sim.V_fuel;        %   mi/gal - Fuel economy
Sim.v_avg = mean(Sim.V_veh);                            %   m/s - Average Velocity

%%  Energy Calculations
Sim.T_wh_tot = Sim.T_wh-Sim.T_brk;
Sim.F_aero = .5*rho_air*A_f*Cd.*Sim.V_veh.^2;     %   N - Aerodynamic force
