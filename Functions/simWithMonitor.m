...
function myTimerFcn(myTimerObj, ~, model,timeStep,minRate)
    ...
    b1 = 'HeavyDutyTruckSim/Vehicle/Vehicle Road Load/Integrator1';
    b2 = 'HeavyDutyTruckSim/Vehicle/Vehicle Road Load/Integrator';
    if stat~= "running"
        fprintf('Simulation status is %s.\n',stat)
    else
        rto1 = get_param(b1,'RunTimeObject');
        rto2 = get_param(b2,'RunTimeObject');
        fprintf('SimTime: %0.2f s;\tVel: %0.1f m/s;\tPos: %0.2f km;\tRunTime: %0.1f s\n',...
            time,rto2.OutputPort(1).Data,rto1.OutputPort(1).Data/1e3,timeStep*(myTimerObj.TasksExecuted-1));
