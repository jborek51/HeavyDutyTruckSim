/* Produced by CVXGEN, 2019-07-30 10:35:06 -0400.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "lam_b");
  if (xm == NULL) {
    printf("could not find params.lam_b.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("lam_b must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter lam_b must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter lam_b must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter lam_b must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.lam_b;
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "lam_e");
  if (xm == NULL) {
    printf("could not find params.lam_e.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("lam_e must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter lam_e must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter lam_e must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter lam_e must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.lam_e;
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 1; i++)
      printf("  params.lam_b[%d] = %.6g;\n", i, params.lam_b[i]);
    for (i = 0; i < 1; i++)
      printf("  params.lam_e[%d] = %.6g;\n", i, params.lam_e[i]);
    for (i = 0; i < 1; i++)
